  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8">
      ...
      <motion.div 
        ...
        className="relative w-full max-w-2xl bg-[#0f0f14] border border-white/10 rounded-[2.5rem] overflow-hidden shadow-2xl"
      >
        ...
        {step === 'customize' ? (
          <div className="flex flex-col">
            {/* Preview Area */}
            <div className="bg-[#050507] p-12 flex flex-col items-center justify-center relative min-h-[300px]">
              ...
              <div className="absolute bottom-8 left-0 right-0 px-8 flex items-center justify-between">
                    ...
                    <AnimatePresence>
                      {isColorPickerOpen && (
                        <motion.div 
                          ...
                          className="absolute bottom-full left-1/2 mb-4 p-4 bg-[#1a1a20] border border-white/10 rounded-2xl shadow-2xl z-50"
                        >
                          <HexColorPicker color={color} onChange={setColor} />
                          <div className="mt-4 flex items-center justify-between bg-black/40 p-2.5 rounded-xl border border-white/5">
                            <div className="flex items-center gap-2">
                              <div className="w-5 h-5 rounded-lg shadow-sm border border-white/10" style={{ backgroundColor: color }} />
                ...
                <div className="relative">
                  <select 
                    ...
                    className="appearance-none bg-white/5 border border-white/10 rounded-xl px-4 py-2 pr-10 text-sm font-bold text-white outline-none cursor-pointer hover:bg-white/10 transition-colors"
                  >
                    {fonts.map(f => <option key={f} value={f} className="bg-[#0f0f14]">{f}</option>)}
                  </select>
            ...
            <div className="p-8 space-y-8">
                  ...
                  <div className="relative group">
                    <div className="absolute left-4 top-1/2 -translate-y-1/2 flex items-center gap-2">
                       <button 
                         ...
                         className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center hover:bg-white/10 transition-colors text-xl"
                       >
                         {emoji}
                       </button>
                    </div>
                    <input 
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-2xl py-5 pl-16 pr-6 text-white font-bold outline-none focus:border-[#f7931a]/50 transition-all"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-white/5">
                <button 
                  ...
                  className={`flex items-center gap-4 p-5 rounded-2xl border transition-all ${type === 'standard' ? 'bg-[#f7931a]/10 border-[#f7931a]/50' : 'bg-white/5 border-white/5 hover:bg-white/[0.08]'}`}
                >
                  ...
                  <div className="text-left">
                    <div className="text-white font-black text-sm">Standard button</div>
                    <div className="text-slate-500 text-xs font-medium">Clean and simple</div>
                  </div>
                </button>

                <button 
                  onClick={() => setType('count')}
                  className={`flex items-center gap-4 p-5 rounded-2xl border transition-all ${type === 'count' ? 'bg-[#f7931a]/10 border-[#f7931a]/50' : 'bg-white/5 border-white/5 hover:bg-white/[0.08]'}`}
                >
                  ...
                  <div className="text-left">
                    <div className="text-white font-black text-sm">Button + supporter count</div>
                    <div className="text-slate-500 text-xs font-medium">Show your social proof</div>
                  </div>
                </button>
              </div>
            </div>
          ...
          <div className="p-12 space-y-10">
             <div className="space-y-4">
                <h3 className="text-3xl font-black text-white tracking-tight">Your code is ready!</h3>
                ...
                <div className="space-y-3">
                   <div className="flex items-center justify-between px-2">
                      ...
                      <button 
                        ...
                        className="text-[10px] font-black uppercase tracking-widest text-[#f7931a] flex items-center gap-2 hover:text-white transition-colors"
                      >
                        {copiedType === 'script' ? <Check size={12} /> : <Copy size={12} />}
                        {copiedType === 'script' ? 'Copied' : 'Copy Code'}
                      </button>
                   </div>
                   <div className="bg-[#050507] p-6 rounded-2xl border border-white/5 font-mono text-xs text-slate-400 break-all leading-relaxed whitespace-pre-wrap">
                      {generatedCode}
                   </div>
                </div>
             ...
             <div className="flex gap-4 pt-4">
                <button 
                  ...
                  className="flex-1 py-4 bg-white/5 border border-white/10 rounded-2xl text-white font-black uppercase tracking-widest text-xs hover:bg-white/10 transition-colors"
                >
                   Back to customize
                </button>
                <button 
                  onClick={onClose}
                  className="flex-1 py-4 bg-white text-black rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-slate-200 transition-colors"
                >
                   Finish
                </button>
             </div>
