  ...
  return (
    <div className="w-full space-y-12">
        ...
        {tools.map((tool, index) => (
          ...
          >
            ...
            <div className={`relative z-10 flex-1 flex flex-col items-center ${tool.locked ? 'opacity-30' : ''}`}>
              <h3 className="text-2xl font-black text-slate-900 dark:text-white mb-4 tracking-tight group-hover:text-[#f7931a] transition-colors">
                {tool.title}
              </h3>
      ...
      <motion.div 
        ...
        className="p-10 rounded-[3rem] bg-gradient-to-br from-slate-50 to-slate-100 dark:from-[#0f0f14] dark:to-[#050507] border border-slate-200 dark:border-white/5 flex flex-col md:flex-row items-center justify-between gap-8 shadow-sm"
      >
         <div className="space-y-4">
            <h2 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Want to learn more?</h2>
            <p className="text-slate-500 font-medium">Our API allows you to build completely special experiences for your fans.</p>
         </div>
         <Link href="/docs/visual-toolkit" target="_blank" rel="noopener noreferrer" className="whitespace-nowrap px-8 py-4 bg-white border border-slate-200 dark:border-transparent text-slate-800 dark:text-black rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-slate-100 dark:hover:bg-slate-200 transition-colors shadow-2xl">
            View API Docs
         </Link>
      </motion.div>
